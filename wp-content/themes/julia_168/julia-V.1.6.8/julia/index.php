				...
				<?php if ( have_posts() ) :
						...
						<?php if ( current_user_can( 'edit_posts' ) ) :
							// Show a different message to a logged-in user who can add posts.		?>
								<h1 class="entry-title"><?php esc_html_e( 'No posts to display', 'julia' ); ?></h1>
								<p><?php printf( __( 'Ready to publish your first post? <a href="%s">Get started here</a>.', 'julia' ), admin_url( 'post-new.php' ) ); ?></p>
							<?php else :
							// Show the default message to everyone else. ?>
								<h1 class="entry-title"><?php esc_html_e( 'Nothing Found', 'julia' ); ?></h1>
								<p><?php esc_html_e( 'Apologies, but no results were found. Perhaps searching will help find a related post.', 'julia' ); ?></p>
