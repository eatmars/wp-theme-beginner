...
$blog_button_prev_text = get_theme_mod( 'blog_button_prev_text' ) ? get_theme_mod( 'blog_button_prev_text' ): esc_html__('PREVIOUS POST', 'julia');
$blog_button_next_text = get_theme_mod( 'blog_button_next_text' ) ? get_theme_mod( 'blog_button_next_text' ): esc_html__('NEXT POST', 'julia'); ?>
<div id="mid_container_wrapper" class="mid_container_wrapper_section"> 	<!--Start Middle Section  -->
<div id="mid_container" class="">
	<?php echo julia_kaya_custom_pagetitle($post->ID); // Page title bar ?>
		...
		<section class="<?php echo esc_attr( $blog_single_page_sidebar ); ?>" id="content_section">
			<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				<div class="single_page_desription_comment_section">
						...
						if(has_post_format()){
							...
							if(has_post_thumbnail()){
								...
								echo '<a href="'.get_the_permalink().'">';
									echo '<img class="blog_single_page_img" src="'.julia_kaya_img_resize($img_url, $img_width, '500', 't').'" class="" alt="'.get_the_title().'" />';    
								echo '</a>';
							}
						} ?>
					...
					<?php if ( get_the_author_meta( 'description' ) ) : // If a user has filled out their description, show a bio on their entries  ?>
						<div id="entry-author-info"> <!-- Author Information -->
							...
							<div id="author-description" class="description">
								<h3><?php printf( esc_attr__( 'Artical Written By %s', 'julia' ), get_the_author() ); ?></h3>
					...
					<?php endif; 
					julia_kaya_relatedpost($post->ID); // Related Postes
