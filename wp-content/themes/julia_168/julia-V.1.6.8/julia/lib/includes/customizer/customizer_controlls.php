<?php
if( !function_exists('julia_kaya_customize_register') ){
	add_action('customize_register', 'julia_kaya_customize_register');
	function julia_kaya_customize_register($wp_customize_manager) {
		...
		$wp_customize_manager->remove_section( 'widgets' );
		$wp_customize_manager->remove_section( 'themes' );
	...
	function julia_kaya_check_number( $input ) {
	    $input = absint( $input );
		return ( $input ? $input : '00' );
	}
	function julia_kaya_opacity_number_validate( $input ){
			return $input;
	}	
	function julia_kaya_text_filed_sanitize( $input ) {
	    return  ( $input ? $input : '&nbsp;' );
	}
	function julia_kaya_input_filed_sanitize( $input ) {
	    return  ( $input );
	}
	function julia_kaya_color_filed_sanitize( $input ) {
	    return $input;
	}
	function julia_kaya_checkbox_field_sanitize( $input ) {
	    if ( $input == 1 ) {
	        return 1;
	    } else {
	        return 0;
	...
	function julia_kaya_radio_buttons_sanitize( $input, $setting ) {
	    global $wp_customize;
	...
	function julia_kaya_sanitize_email( $email ) {
		if(is_email( $email )){
			return $email;
		}else{
			return '';
...
if( !function_exists('julia_kaya_controls_register') ){
	function julia_kaya_controls_register( $wp_customize ){
	// border customizer controll
	if(class_exists('WP_Customize_Control')){		
		class Julia_Kaya_Customize_Divider_Control extends WP_Customize_Control{
			public $type = 'divider';
	...
	if(class_exists('WP_Customize_Control')){	
		class Julia_Kaya_Customize_Sliderui_Control extends WP_Customize_Control {
			public $type = 'slider';
			public function julia_kaya_enqueue() {
				...
				<script>
					jQuery(document).ready(function($) {
						$( '[id="slider_<?php echo esc_attr( $this->id ); ?>"]' ).slider({
							value : <?php echo trim( $this->value() ) ?  str_replace('&nbsp;', '0', $this->value()) : '0'; ?>,
	...
	if(class_exists('WP_Customize_Control')){
		class Julia_Kaya_Customize_Imageupload_Control extends WP_Customize_Control {
			...
			public function render_content(){
		        ...
		        <label>
		        	...
		        	<div class="customize_img_upload">
						...
						<input type="hidden">
						<div class="buttons">
							<a href="#" class='upload_media_image button'><?php esc_html_e('Upload Image','julia') ?></a>
							<a href="#" class='remove_media_image button'><?php esc_html_e('Remove','julia') ?></a>
						</div> 
					</div>
		        </label>	        
		        <?php
	...
	if(class_exists('WP_Customize_Control')){
		class Julia_Kaya_Customize_Textarea_Control extends WP_Customize_Control {
	...
	if(class_exists('WP_Customize_Control')){	
		class Julia_Kaya_Customize_Description_Control extends WP_Customize_Control {
	...
	if(class_exists('WP_Customize_Control')){
		class Julia_Kaya_Customize_Images_Radio_Control extends WP_Customize_Control {
...
class Julia_Kaya_Customize_Sidebar_Control extends WP_Customize_Control
{
	...
	*/
	if(class_exists('WP_Customize_Control')){
		class Julia_Kaya_Customize_Page_Control extends WP_Customize_Control
		{
			private $pages = false;
			public $type = 'page';
			...
			public function render_content()
		    {
		        if(!empty($this->pages))
		        {
		            ?>
		                <label class="customize-control-title"><?php echo esc_html( $this->label ); ?></label>
		                  <select <?php $this->link(); ?> name="<?php echo $this->id; ?>" id="<?php echo $this->id; ?>">
                    <option value="select-page"><?php _e('Select Page','gilda') ?></option>
                    <?php
		...
		class Julia_Kaya_Customize_google_fonts_Control extends WP_Customize_Control 
		{
			...
			public function render_content(){ ?>
				...
				<select <?php $this->link(); ?> style="">
					<?php
					$defaylt_fonts = array ('0' => __('Select Font','julia'),
