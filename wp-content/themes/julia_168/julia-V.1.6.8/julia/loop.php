...
$kaya_readmore_blog=get_theme_mod('kaya_readmore_blog') ? get_theme_mod('kaya_readmore_blog')  : esc_html__('Read More', 'julia');
$blog_post_date_title_color = get_theme_mod('blog_post_date_title_color') ? get_theme_mod('blog_post_date_title_color') : '#ffffff';
$blog_post_date_bg_color = get_theme_mod('blog_post_date_bg_color') ? get_theme_mod('blog_post_date_bg_color') : '#ff3333';
$blog_cat_limit_content = get_theme_mod('blog_cat_limit_content') ? get_theme_mod('blog_cat_limit_content') : '10';
 while ( have_posts() ) : the_post(); ?>
  <article <?php post_class('standard-blog'); ?> >
   <div class="blog_post_content_wrapper">
    ...
    <?php     
      ...
      if(has_post_format()){
        ...
        if(has_post_thumbnail()){
            echo '<img src="'.julia_kaya_img_resize($img_url, '', '', 't').'" class="" alt="'.get_the_title().'" />';
            $class="two_third_last";
            $post_date_wrapper = '';
            $button_align_left='';
            $img_border='';
      ...
      echo '<div class="meta_date_wrapper '.$post_date_wrapper.'">';
        echo '<div class="meta_date"><span class="meta_date_day">'.get_the_date('d').'</span><span class="meta_date_month">'.get_the_date('M').'</span></div>';
      echo '</div>';   ?>
    </div>
    <?php if( get_post_format() == 'quote' ){
        ...
        echo '<p>'.wp_trim_words( get_the_content(), $blog_cat_limit_content, '' ).'</p>';
        echo '<div class="meta_post_info">';
        echo '<span class="mata_author"><i class="fa fa-user"></i>'. __('Written by ', 'julia') .'<a href="'. get_author_posts_url( get_the_author_meta( 'ID' ) ).'">'.get_the_author().'</a></span>'; ?>
        <span class="meta_category"><i class="fa fa-tags"></i><?php esc_html_e('Tagged as ', 'julia')?> <?php echo the_category(', '); ?> </span> 
        </div>
  ...
  <?php echo julia_kaya_pagination(); ?>
